import 'package:apro/presentation/constant/theme.dart';
import 'package:flutter/material.dart';

class DataKapalTable extends StatelessWidget {
  const DataKapalTable({super.key});

  @override
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        backgroundColor: Colors.grey[100],
        appBar: AppBar(
          backgroundColor: Colors.transparent,
            title: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(
                      'Data Kapal',
                      style: boldBlackTextStyle.copyWith(fontSize: 18),
                    ),
                  ],
                ),
              ],
            ),
        ),
        body: Container(
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(8.0),
            boxShadow: [
              BoxShadow(
                color: Colors.grey.withOpacity(0.5),
                spreadRadius: 2,
                blurRadius: 5,
                offset: const Offset(0, 3), // mengatur posisi bayangan
              ),
            ],
          ),
          child: SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Padding(
              padding: const EdgeInsets.fromLTRB(12, 12, 12, 25),
              child: DataTable(
                columns: const [
                  DataColumn(label: Text('Nama Kapal')),
                  DataColumn(label: Text('Milik')),
                  DataColumn(label: Text('Panjang Kapal')),
                  DataColumn(label: Text('Sarat Muka Kapal')),
                  DataColumn(label: Text('Sarat Belakang Kapal')),
                  DataColumn(label: Text('Dwt')),
                  DataColumn(label: Text('Datang')),
                  DataColumn(label: Text('Tujuan')),
                  DataColumn(label: Text('Bendera Kebangsaan ')),
                  DataColumn(label: Text('Jenis Kapal')),
                ],
                rows: const[
                  DataRow(
                    cells: [
                      DataCell(Text('JOSEPHINE III')),
                      DataCell(Text('-')),
                      DataCell(Text('-')),
                      DataCell(Text('-')),
                      DataCell(Text('-')),
                      DataCell(Text('-')),
                      DataCell(Text('Jakarta')),
                      DataCell(Text('Sabang')),
                      DataCell(Text('-')),
                      DataCell(Text('-'))
                    ],
                  ),
                ],
              ),
            ),
          ),
        ),
      ),
    );
  }
}